              ...
              {expanded && (
                <div className="border-t border-sentinel-line bg-sentinel-bg/70">
                  {group.classes.map((item) => {
                    ...
                    const primaryLabel = item.displayLabel || item.label;
                    const llmPrimary = item.labelSource === 'llm_advisory';
                    const advisory = item.llmAdvisory;
                    const advisoryLabel = !llmPrimary && advisory?.label && advisory.label !== primaryLabel ? advisory.label : null;
                    const advisoryTitle = advisory
                      ? `AI suggestion: ${advisory.label || ''}${advisory.description ? ` — ${advisory.description}` : ''}\nGenerated by ${advisory.generated_by || 'llm'}. Raw class: ${item.rawClass}. Deterministic category/threat remain canonical.`
                      : '';
                    return (
                      <div key={item.rawClass} className="grid grid-cols-[22px_18px_1fr_auto_auto] items-center gap-2 px-3 py-1.5">
                        ...
                        <button type="button" className="min-w-0 text-left" onClick={() => soloDetectionClass(item.rawClass)}>
                          <span className={`block truncate text-[11px] ${hidden ? 'text-sentinel-muted' : 'text-slate-200'}`} title={llmPrimary ? advisoryTitle : undefined}>
                            {primaryLabel}{solo ? ' / SOLO' : ''}
                            {advisoryLabel && (
                              <span
                          ...
                          {llmPrimary && (
                            <span className="block truncate font-mono text-[9px] uppercase tracking-wider text-sentinel-muted">
                              raw / {item.rawClass}
                            </span>
                          )}
                        </button>

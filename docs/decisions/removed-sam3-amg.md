...
AMG produces masks but **no labels**. To get class names for the masks, every detection had to be re-classified by a second model (Grounding-DINO). That meant:
...
YOLOE-26x-seg(-pf) does all three in one forward pass — it emits `(mask, bbox, label, score)` natively. The `-pf` head is the prompt-free replacement; the `-seg` head replaces AMG + text prompts.

## Migration

Anything still sending `prompt_mode=amg` to `/detect_video` gets HTTP 400 from inference-sam3. Operators should use `prompt_mode=yoloe` instead:

...
[backend/worker_legacy.py](../../backend/worker_legacy.py) is ~3650 lines, the largest single file in the repo. It is **not** being split incrementally. Instead, a thin package [backend/worker/](../../backend/worker/) re-exports it.
...
- **Coverage is in the integration paths.** The unit tests in `backend/tests/` cover small modules (auth, ontology, candidate linking) — `worker_legacy.py` is covered mostly by end-to-end runs and bench scripts. Splitting it into 12 modules without a test net would lose that integration coverage by accident.
...
- **Performance.** A single import of `worker_legacy` is faster than 12 module imports at Celery worker startup. Trivial in absolute terms but real in practice for cold restarts.
...
[backend/worker/dispatch.py](../../backend/worker/dispatch.py), `imagery.py`, `fmv.py`, `postprocess.py` each re-export a curated subset for new code that wants narrow imports. They contain **no logic** — only re-exports and one or two thin wrappers that constants live in.

This means: new code can do `from worker.imagery import chip_plan` (clean) while existing callers keep `from worker_legacy import chip_plan` (legacy-friendly). The legacy import path will be removed only after callers migrate.

...
- **Open-vocabulary segmentation.** SAM3's native API accepts free-text prompts and produces pixel masks. Closed-vocab specialists (DOTA-OBB, YOLOE-pf) emit boxes from a fixed list — SAM3 fills the gap for anything not in those lists.
- **Single API across modalities.** RGB, multispectral, and SAR all go through SAM3 — multispectral via Prithvi-derived RGB preview, SAR via TerraMind S1→S2 synthesis. The downstream record schema is identical regardless of sensor.
...
- **Native multiplex video.** SAM 3.1's `build_sam3_multiplex_video_predictor` tracks a single text prompt per session across frames. The worker fans out one request per prompt and merges streams to get multi-class FMV tracking.
...
- **Cannot free without process restart.** This is the architectural reason inference runs as a separate container — see [component-boundaries.md](../architecture/component-boundaries.md) §2.

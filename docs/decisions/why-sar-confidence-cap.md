...
Every detection on a `modality=sar` chip has its `confidence` **hard-capped at `SAM3_SAR_CONF_CAP=0.85`**, flagged `sar_proxy=true`, `review_status="review_candidate"`.

## Why

SAR is not segmented directly. Sentinel-1 GRD comes in as 2-band VV/VH (dB-clipped to [-30, 0], then linearly stretched to [0, 1]). [TerraMind v1](../inference/terramind-sar-synthesis.md) generates a **synthetic S2-L2A optical preview** from the SAR bands; SAM3 runs on that synthetic RGB.

The synthetic preview is **not** ground truth — a plausible optical reconstruction conditioned on the SAR backscatter. Two consequences:
...
2. **Operators must review SAR detections by hand** with the actual SAR raster before acting.
...
A separate SAR-native ship detector (CFAR — see [backend/sar-cfar-detector.md](../backend/sar-cfar-detector.md)) runs independently of SAM3, does not go through TerraMind. CFAR detections are *not* SAR-proxy — they're real backscatter peaks. They appear alongside SAM3-via-TerraMind detections in the imagery pipeline. Don't conflate.

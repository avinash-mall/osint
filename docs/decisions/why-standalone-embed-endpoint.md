...
- **Don't pay the full detection cost.** `POST /detect` runs SAM3 + DOTA-OBB + MVRSD, fusion, and enrichment — 100-500x the compute the bake actually needs. Calling `/detect` to harvest one embedding field would burn GPU time and add latency for no gain.

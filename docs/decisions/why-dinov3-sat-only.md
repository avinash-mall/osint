...
Benchmarks (see [benchmarks/embedding-stability.md](../benchmarks/embedding-stability.md) and [benchmarks/video-tracking-stability.md](../benchmarks/video-tracking-stability.md)):
...
DINOV3_LVD emitted **NaN embeddings** on real drone-video crops — a silent failure that broke downstream cosine similarity without raising. It was also 2.5× slower with no measured quality advantage on either still or video data. See [removed-dinov3-lvd.md](removed-dinov3-lvd.md).

DINOv3-SAT is specifically pretrained on satellite imagery (SAT-493M), which is the dominant modality in the stack and dominates the re-ID workload (cross-pass detection linking).

## What this enables

- **Cross-image re-ID.** `/api/detections/{id}/similar` returns embedding-cosine nearest neighbors across all stored detections.
- **Cross-frame re-ID.** FMV tracks carry a single embedding per track (first frame); used by candidate-link scoring and the "Similar" panel in the UI.

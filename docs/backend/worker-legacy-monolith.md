# Worker Tasks — Runtime Behavior Reference
...
> **The 6.2k-line `worker_legacy.py` monolith was split into the `worker` package**
> (2026-06-16; see [worker-package-facade.md](worker-package-facade.md) for the module
> map and [decisions/why-worker-package-split-2026-06-16.md](../decisions/why-worker-package-split-2026-06-16.md)).
> `worker_legacy.py` is now a 13-line compatibility shim. This doc is the **runtime
> behavior** reference (what the tasks do, fork safety, failure modes); links below point
> to the owning package modules.
...
Tasks and helpers are split by concern across [`backend/worker/`](../../backend/worker/) —
`imagery.py`, `fmv.py`, `graph.py`, `maintenance.py`, `dispatch.py`, `postprocess.py`,
`config.py`, `app.py`, `_shared.py`. Celery task `name=` routing keys are unchanged; a parity
test pins the registry. See [worker-package-facade.md](worker-package-facade.md).
...
- [`_remote_imagery_allowed`](../../backend/worker/) — validates remote imagery hosts before worker-side HTTP(S) fetch.
- [`resolve_input_path`](../../backend/worker/) — resolves staged local paths and gated remote URLs into an input path.

...
- [`_log_unknown`](../../backend/ontology.py#L296) — UPSERT into `ontology_unknown_labels` (count stays exact, every call) and enqueue `worker.project_unknown_labels` to refresh the Neo4j triage mirror. The enqueue is **throttled per label** via [`_should_enqueue_projection`](../../backend/ontology.py#L282) (process-local, once per label per 5-min window): `_log_unknown` runs once per detection during `store_detections`, so a dense pass with thousands of unknown-label detections would otherwise flood the worker with thousands of redundant projection tasks for the same handful of labels. The mirror only needs one projection per label.
- [`normalize`](../../backend/ontology.py#L333) — main entry point.

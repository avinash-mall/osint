...
Bin a detection's class into one of a small set of categories (`air`, `maritime`, `ground`, `infrastructure`, `person`, `animal`, `vegetation`, `water`, `object`) and return a (currently neutral) threat level. Used by the map UI to drive icon and color choice.

## Why this design

- **Open-vocab neutral.** `threat_level` defaults to `"unrated"` for every detection. The platform's stance is that automated threat assignment is out of scope without explicit operator-loaded rules. See [decisions/why-open-vocabulary.md](../decisions/why-open-vocabulary.md).
- **TTL-cached rule lookup.** `_lookup_threat_rule` reads from `threat_rules` (PostGIS) but caches by `(class, sensor)` for 60 seconds. Without the cache, every detection-emit call would re-read.
- **Category, not threat, is the load-bearing concept.** The UI uses the category to pick an icon style; the optional threat rules layer on top.
...
- [`category_for_class`](../../backend/threat_assessment.py#L165) — the bucket assignment.

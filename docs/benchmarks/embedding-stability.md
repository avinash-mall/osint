...
For each ground-truth instance, generate N augmented crops (random rotation, jitter, color), compute embeddings, and check whether the top-1 nearest neighbor in the full pool is the same instance. Reported as top-1 accuracy.
...
The 100% number reflects an internally curated eval; real cross-pass re-ID is more variable. The test is **purpose-built to detect silent failures** (NaN, semantic drift) rather than to certify a particular quality level.

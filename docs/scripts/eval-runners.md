...
Tests the candidate-link scorer ([backend/candidate-linking.md](../backend/candidate-linking.md)) against curated ground-truth pairs. No live backend needed — directly imports `backend.candidate_linking.rank_candidate_links` and runs against `scripts/eval_datasets/candidate_links_gt.json`.
...
Smoke test for [backend/sar-cfar-detector.md](../backend/sar-cfar-detector.md). Runs CFAR on the synthetic SAR fixture set and reports detection count + score distribution.

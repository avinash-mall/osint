...
- [`health`](../../inference-sam3/main.py#L944) — `GET /health` (includes `serialize_forwards` and `degraded_layers`). `degraded_layers` (list[str]) names specialist layers that were requested but failed to load — any of `mvrsd`/`dota_obb` whose `model_versions[layer].error` is set (e.g. `mvrsd` when its weight is missing). Empty when the active profile simply omits the layer.
...
- **Cross-tile NMS mode** (`SAM3_NMS_AGNOSTIC` / `SAM3_NMS_SOFT`, read in `_detect_pipeline`) — selects the post-decode dedup. The code defaults flipped toward dense-scene recall: `SAM3_NMS_AGNOSTIC` `1→0` (now **per-class**, so adjacent boxes of different classes no longer cross-suppress) and `SAM3_NMS_SOFT` `0→1` (now **soft**, decaying overlaps by `(1 − mask_iou)` instead of dropping them). Soft mode forces the `fusion.mask_aware_nms` path; otherwise dispatch goes through the env-selectable fuser (`SAM3_FUSION_MODE`). See [decisions/dense-scene-recall-defaults.md](../decisions/dense-scene-recall-defaults.md).
...
- [decisions/dense-scene-recall-defaults.md](../decisions/dense-scene-recall-defaults.md) — NMS-mode flip (per-class + soft)

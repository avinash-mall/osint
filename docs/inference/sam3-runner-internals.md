...
- [`_cuda_context_poisoned`](../../inference-sam3/sam3_runner.py#L1014) — classifies a chunk exception as an unrecoverable CUDA fault (illegal memory access / device-side assert / cuBLAS-cuDNN init failure, excluding recoverable OOM). Drives the `os._exit(1)` self-heal in the cached-batched chunk loop. See [decisions/why-exit-on-poisoned-cuda-context.md](../decisions/why-exit-on-poisoned-cuda-context.md).
...
[`run_video_yoloe`](../../inference-sam3/sam3_runner.py#L1534)'s per-frame `try/except` keeps streaming past a transient `yoloe.run` failure, but classifies the exception via `_cuda_context_poisoned` first: an unrecoverable CUDA fault used to be swallowed into `candidates = []` for every remaining frame — a healthy-looking zombie emitting zero detections — and now logs critical + `os._exit(1)` like the imagery pipeline ([`yoloe.run`](../../inference-sam3/yoloe.py#L148) re-raises poisoned faults instead of returning `[]` so this guard can see them). The `/detect_video` stream generators in `main.py` carry the same check for the PCS path. See [decisions/audit-fixes-inference-2026-06-12.md](../decisions/audit-fixes-inference-2026-06-12.md).

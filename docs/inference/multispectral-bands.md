...
Decode the 6-band HLS / S2-L2A GeoTIFF payload that the worker sends for multispectral chips, produce an RGB preview, and resize to Prithvi's input shape.
...
Prithvi `constant_scale=0.0001` is applied: HLS reflectance is uint16 (`0..10000`) scaled to float32 (`0..1`).

## Key symbols

- [`decode_hls6`](../../inference-sam3/multispectral.py#L13) — bytes (multipart payload) → `(H, W, 6)` float32.
- [`hls_to_rgb_preview`](../../inference-sam3/multispectral.py#L21) — picks bands 3/2/1 (RGB) and stretches for SAM3.

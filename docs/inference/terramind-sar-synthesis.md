...
- [`pool_patches`](../../inference-sam3/terramind.py#L54) — TerraMind embedding pooling (used when `metadata.terramind_embedding` requested).
- [`_fallback_sar_rgb`](../../inference-sam3/terramind.py#L73) — TerraMind not loaded → naive false-color RGB so the path still produces a navigable preview.

...
2. **Create the runner module** `inference-sam3/<name>.py` exposing:
   - `load(device) -> bundle: dict`
   - `run(bundle, image, prompts) -> list[(mask, bbox_xyxy, score, label)]` — matching existing detectors' signature so [fusion.py](../../inference-sam3/fusion.py) can ingest results.
   - `model_versions(bundle) -> dict` — surfaced in `/health`.

3. **Register a load flag** `SAM3_LOAD_<NAME>` in [inference-sam3/main.py](../../inference-sam3/main.py). Wire into `_build_component` (`#L464`) and `_load_profile` (`#L521`).
...
6. **Verify with the benchmark harness** before promoting:
   ...
   If the new model only **adds false positives** without adding true positives, **do not ship**. See [decisions/removed-defence-yolo.md](../decisions/removed-defence-yolo.md) (1297 FPs / 0 TPs).

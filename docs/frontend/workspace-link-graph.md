...
- **Image + Class scope dropdowns.** Two combinable `<select>`s replace the old fixed node limits: **Image** (from `GET /api/graph/passes` — imagery scenes with detection counts + acquisition date) and **Class scope** (from `GET /api/graph/classes` — detection classes + counts). Selecting either (or both, ANDed) fetches the *unbounded* scoped graph (`GET /api/graph?pass_id=…&det_class=…` — the matching detections + their 1-hop neighbourhood) as a filtered view, and re-scopes the metrics card and co-location lens to the same selection. "All images" + "All entities" returns the operational overview for the canvas and **whole-graph** metrics. "Clear scope" / "Exit lens" / the toolbar "Clear" reset both. See [decisions/why-class-scope-replaces-node-limit.md](../decisions/why-class-scope-replaces-node-limit.md).

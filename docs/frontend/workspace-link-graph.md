...
- **Class scope dropdown.** A **Class scope** `<select>` (populated from `GET /api/graph/classes` — distinct detection classes + counts) replaces the old fixed node limits. Picking a class fetches the *unbounded* class-scoped graph (`GET /api/graph?det_class=…` — every detection of that class + its 1-hop neighbourhood) as a filtered view, and re-scopes the metrics card (`?det_class=…`) and the co-location lens to that class. "All entities" returns the operational overview for the canvas and **whole-graph** metrics. "Clear scope" / "Exit lens" / the toolbar "Clear" all reset it. See [decisions/why-class-scope-replaces-node-limit.md](../decisions/why-class-scope-replaces-node-limit.md).

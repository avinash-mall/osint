...
- [frontend/src/components/map/DetectionTileLayer.tsx](../../frontend/src/components/map/DetectionTileLayer.tsx) (~140 lines; opt-in Martin MVT detection layer behind `VITE_DETECTION_TILES`)
...
- **MVT detection tiles (opt-in, `VITE_DETECTION_TILES='1'`)** — `DetectionTileLayer.tsx` renders persisted detections as Martin vector-tile polygons via `leaflet.vectorgrid` (`L.vectorGrid.protobuf` on `GET /maps/detections_mvt/{z}/{x}/{y}?v=<version>`, layer `detections`). It is an **imperative** react-leaflet layer (VectorGrid has no JSX wrapper): an effect attaches/detaches the layer on the parent map and re-creates it whenever the cache-bust `version` or any client-side filter changes (Phase 2 — no per-feature `setFeatureStyle`). `styleForTileProps` reproduces `makeDetectionStyle` from the tile's own `branch_id` (the same value `branchIdForFeature` returns) → `categoryFor` colour, confidence opacity, `Military_Forces` dash, heavy outline — and applies the **same** confidence-threshold / SOLO-class / hidden-category filters as `filteredDetectionsGeoJSON`, returning `{stroke:false,fill:false}` for filtered-out features. A click reads the tile feature's `id` and fetches `GET /api/detections/{id}/enriched` (same ~39-prop shape as `/api/detections/geojson`), feeding the same `setSelectedDetection` the boxes use. The flag is `USE_DETECTION_TILES` in `MapStage.tsx`; when **on**, the per-feature `<Polygon>` box layer is skipped and the tile layer renders instead — markers/dots and the live `detections_partial` preview stay on the existing GeoJSON path (tiles are static; `detections_updated` bumps `detectionTileVersion` in GaiaMap so persisted tiles refresh after an ingest/delete). **Default is OFF** — the box `<Polygon>` path is unchanged. See [decisions/why-detection-mvt-tiles.md](../decisions/why-detection-mvt-tiles.md).

...
- [frontend/src/components/tour/tourSteps.ts](../../frontend/src/components/tour/tourSteps.ts) (~389 lines)
- [frontend/src/components/tour/ProductTour.tsx](../../frontend/src/components/tour/ProductTour.tsx) (~366 lines)

**Lines:** ~846 total across three files
...
> **2026-06-13 UI revamp anchor changes:** removed `analytics-tools`;
> `tab-provenance`→`details-provenance`; `tm-conf`→`filter-conf`;
> `event-windows`→`filter-time-window`; `event-timeline` retitled "Activity
> sparkline"; `tab-satellites` is now a panel-header affordance (anchor
> preserved). `onStepChange` opens the left panel for `filter-*` steps and
> Details for `details-provenance`. See
...
> [analytics-relocated-to-right-tab](../decisions/analytics-relocated-to-right-tab.md).
...
3. **Step-by-step tooltip walkthrough.** 48 declarative steps (one per interactive control on the page) covering the left rail, filter surface, top action bar, zoom cluster, time-machine controls, bottom activity chrome, and SelectionPanel deep links (Details/Analytics/Similar/Tracks plus gated Satellites and provenance disclosure). Each step body is a two-sentence what-it-does + when-an-analyst-reaches-for-it explanation.
...
### `tourSteps.ts`
- `TOUR_STEPS: TourStep[]` ([tourSteps.ts#L29-L389](../../frontend/src/components/tour/tourSteps.ts#L29-L389)) — 48 steps in display order. Selectors are `[data-tour="..."]` matches.

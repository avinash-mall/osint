...
The chip knobs below are our SAHI-style sliced inference; see
[inference/sahi-equivalence.md](../inference/sahi-equivalence.md) for the SAHI↔env mapping and how to
enable + benchmark the small-object boost.
...
| `MVRSD_WEIGHTS_URL` | (empty) | **Build ARG.** GitHub release-asset URL for the MVRSD weight; baked to `/models/mvrsd/mvrsd_yolo11m.pt` at build time (hard rule #8). Empty = bake skipped + runner honour-gates. Set by the orchestrator after upload. The MVRSD repo is **private**, so this is the GitHub **API** asset endpoint and `GITHUB_TOKEN` must authenticate the fetch — a missing/wrong token 404s and is silently swallowed (layer loads-but-empty). |
| `GITHUB_TOKEN` | empty | **Build ARG** (read from the build host's environment by `docker-compose.yml`; **not currently listed in `.env.example`** — export it in the build shell). Bearer token used by the `Dockerfile.gpu` MVRSD bake to fetch the private-repo release asset at `MVRSD_WEIGHTS_URL` (`curl -H "Authorization: Bearer …"`). Empty/invalid → the authenticated `curl` 404s, the `\|\| echo … failed` fallback swallows it, the build still succeeds, and `/models/mvrsd/` stays empty so `mvrsd.load()` honour-gates (zero candidates). Never commit a real token. |
...
- **GPU division (generated):** `SAM3_VISIBLE_DEVICES`, `SAM3_SERIALIZE_FORWARDS` (multi-replica only), `INFERENCE_CHIP_CONCURRENCY`/`INFERENCE_MIN_PENDING_CHIPS` (SAM3-allocated count).

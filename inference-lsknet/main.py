...
LSKNET_GPU_PROFILE = os.getenv("LSKNET_GPU_PROFILE", "unknown")
...
def _auto_cuda_devices(torch_module: Any) -> list[str]:
    ...
    if unsupported:
        message = (
            f"[INFERENCE-LSKNET] No visible CUDA device has an arch in the torch build "
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if devices:
                ...
                print(
                    f"[INFERENCE-LSKNET] Using CUDA devices {', '.join(devices)}: "
                    f"{', '.join(names)}"
                )
                return devices
    except Exception:
...
def load_model() -> None:
    ...
    with load_lock:
        ...
        try:
            ...
            model_error = str(exc)
            print(f"[INFERENCE-LSKNET] Model prerequisites failed: {exc}")
            return

        for device in DEVICES:
            try:
                model = init_detector(LSKNET_CONFIG, LSKNET_CHECKPOINT, device=device)
                ...
                model_error = str(exc)
                print(f"[INFERENCE-LSKNET] Model load failed on {device}: {exc}")
...
def run_inference_tiled(image_array: np.ndarray, image_size: tuple[int, int], metadata: dict | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"LSKNet tiled inference failed: {exc}") from exc
    ...
    return {
        ...
        "devices": DEVICES,
        "gpu_model": GPU_MODEL,
        "gpu_profile": LSKNET_GPU_PROFILE,
